...
def buy():
    ...
    if request.method == "GET":
        ...
        user_id = session["user_id"]
        user_cash_db = db.execute("SELECT cash FROM user WHERE id = :id", id=user_id)
...
def history():
    ...
    return render_template("history.html", transactions = transactions.db)
...
def add_cash():
    ...
    if requestr.method == "GET":
        ...
        if not new_cash:
            return apology("You must provide money")
...
def register():
    ...
    if request.method == "GET":
        ...
        if not username:
            return apology("Provide the username")

        if not password:
            return apology("Provide the password")

        if not confirmation:
            return apology("Must provide the confirmation to the password")
